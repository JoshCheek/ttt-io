...
describe("BoardAnalyzer") do(
  ...
  it("knows about vertical wins", block(
  ...
  it("knows about diagonal wins", block(
  ...
  it("identifies the correct winner", block(
    BoardAnalyzer forBoard(Board forString("111220000")) winner will == 1
    BoardAnalyzer forBoard(Board forString("222110000")) winner will == 2
  ))
  
  it("does not find a winner when there isn't one", block(
    BoardAnalyzer forBoard(Board forString("112221121")) winner will == nil
  ))
  
  it("knows there is not a tie when there is a winner", block(
    BoardAnalyzer forBoard(Board forString("112122121")) isTie will == false
  ))
  
  it("knows there is not a tie when there are spaces left to move in", block(
    BoardAnalyzer forBoard(Board forString("112122021")) isTie will == false
  ))
  
  it("knows there is a tie when there is nowhere to move and no winner", block(
